...
var _cssParse = function(style){
	if(!_.isString(style)) return {};

	var list = style.toLowerCase().split(';');
	var result = {};
	list.forEach(function(cell, i){
		var item = cell.split(":");
		if(item.length !== 2) return;
		var name = utils.dasherize(item[0]).trim();
		result[name] = item[1].trim();
	});
	return result;
}

var _maybeAddPx = function(name, value) {
	return (typeof value == "number" && !cssNumber[utils.dasherize(name)]) ? value + "px" : value;
}

var css = exports.css = function(property, value){
	if (!this[0]) return this;

	//get property value
	if (_.isString(property) && typeof value === 'undefined'){
		var styles = _cssParse(this.attr('style'));
	...
	return this.each(function(i, el){
		...
		for(var key in newStyle){
			var newKey = utils.dasherize(key);
			var newValue = _maybeAddPx(newKey, newStyle[key]);

			//delete property
			if(!newValue) {
				delete styles[newKey];
			}else{
				styles[newKey] = newValue;
			};
		};

		var styleText = JSON.stringify(styles)
			.replace(/["}{]/g, '')
			.replace(/,/g, ';');
...
var prop = function(name, value){
	//async|autofocus|checked|location|multiple|readOnly|selected
	if(name )
