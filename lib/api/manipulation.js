...
function uniqueSplice(array, spliceIdx, spliceCount, newElems, parent) {
  ...
  const next = array[spliceIdx + spliceCount] || null;
  ...
  for (let idx = 0; idx < newElems.length; ++idx) {
    ...
    const prevIdx = oldParent && oldParent.children.indexOf(newElems[idx]);
...
exports.wrapAll = function (wrapper) {
  if (this[0]) {
    ...
    const wrap = this._make(wrapper).insertBefore(this[0]);

    // If html is given as wrapper, wrap may contain text elements
    let elInsertLocation = { children: wrap };
    let j = 0;
...
exports.insertAfter = function (target) {
  ...
  self.remove();
  domEach(target, (i, el) => {
    const clonedSelf = self._makeDomArray(self.clone());
    const { parent } = el;
    if (!parent) {
      return;
    }
...
exports.html = function (str) {
  ...
  const opts = Object.apply({}, this.options); // Keep main options
...
exports.text = function (str) {
  ...
  return domEach(this, (_, el) => {
    ...
    const textNode = new domhandler.Text(str);
