...
var nextUntil = exports.nextUntil = function(selector, filter) {
  if (!this[0]) { return this; }
  var elems = [], elem = this[0], untilNode, untilNodes;
  if (typeof selector === 'string') {
    untilNode = select(selector, this.nextAll())[0];
  ...
  if (!selector || !selector.cheerio) {
    ...
    while ((elem = elem.next)) {
      if (isTag(elem) && untilNodes.indexOf(elem) > -1) {
        break;
      } else {
        elems.push(elem);
      }
  ...
  return this.make(filter ? select(filter, elems) : elems);
};
...
var prevUntil = exports.prevUntil = function(selector, filter) {
  ...
  var elems = [], elem = this[0], untilNode, untilNodes;
  if (typeof selector === 'string') {
    untilNode = select(selector, this.prevAll())[0];
  } else if (selector && selector.cheerio) {
    untilNodes = selector.toArray();
  } else if (selector) {
    untilNode = selector;
