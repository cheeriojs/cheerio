...
var nextUntil = exports.nextUntil = function(selector, filter) {
  if (!this[0]) { return this; }
  var elems = [], elem = this[0], nextAll = this.nextAll();
  while ((elem = elem.next)) { 
    if (isTag(elem) && elem !== select(selector, nextAll)[0]) {
      elems.push(elem);
    } else {
      break;
    }
...
var prevUntil = exports.prevUntil = function(selector, filter) {
  if (!this[0]) { return this; }
  var elems = [], elem = this[0], prevAll = this.prevAll();
  while ((elem = elem.prev)) { 
    if (isTag(elem) && elem !== select(selector, prevAll)[0]) {
      elems.push(elem);
    } else {
      break;
    }
