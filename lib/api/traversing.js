...
var parents = exports.parents = function(selector) {
  ...
  parents = traverseParents(this, elem, selector);

  if (parents.length) {
    return selector ? this.make(parents).filter(selector) : this.make(parents)
  }
...
var closest = exports.closest = function(selector) {
  ...
  return traverseParents(this, this[0], selector, true);
}
...
function traverseParents(self, elem, selector, earlyTermination) {
  var elems = [];

  while (elem.type != 'root') {
    if (earlyTermination) {
      elems = self.make(elem).filter(selector);
      if (elems.length) {
        return elems;
      }
    } else {
