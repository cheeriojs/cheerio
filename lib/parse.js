...
var evaluate = exports.evaluate = function(content, options) {
  ...
  _.forEach(handler.dom, function(el) {
    parseData(el, content, options);
  });
 ...
 */
var parseData = exports.parseData = function(elem, content, options) {
  if (elem.data === undefined) elem.data = {};
  var value;
  for (var key in elem.attribs) {
    value = decode(elem.attribs[key]);

    // If the decoded value differs then do not render the cached html
    if (value !== elem.attribs[key]) {
      utils.invalidate(elem);
    }
  ...
  // Pull in cached html if possible
  if (elem._endIndex && (!options || (!options.lowerCaseTags && !options.normalizeWhitespace))) {
    // Check for /> mismatch and invalidate if we see this case
    if (content.indexOf('/>', elem._endIndex - 2)) {
      utils.invalidate(elem);
    } else {
      elem._html = content;
    }
  ...
  _.forEach(elem.children, function(el) {
    parseData(el, content, options);
  });
};
